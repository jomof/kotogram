...
def test_mlm_grammar_only():
    ...
    for field in ['surface', 'lemma']:
        ...
        assert (current_ids[attention_mask] == mask_token_id).all(), f"Field {field} should be 100% MASKED"
        
        # Labels should be ALL -100 (ignored)
        labels = masked_batch[f'mlm_labels_{field}']
        assert (labels == -100).all(), f"Field {field} labels should be all -100 (ignored)"
    ...
    for field in ['pos', 'pos_detail1', 'conjugated_type']:
        ...
        assert not (current[attention_mask] == mask_token_id).all(), f"Field {field} should NOT be 100% masked"
