...
class TestDisallowList:
    ...
    def test_initialize_disallow_filter_caches_ids(self):
        ...
        feature_ids = {"pos_detail_1": [10, 42, 20, 42]}  # 42 is noun:common-noun
    ...
    def test_get_disallowed_positions_returns_empty_if_not_initialized(self):
        ...
        feature_ids = {"pos_detail_1": [1, 2, 3]}
        positions = get_disallowed_positions(feature_ids)
        assert isinstance(positions, set)

    def test_get_disallowed_positions_returns_empty_without_pos_detail_1(self):
        """Verify get_disallowed_positions returns empty for missing field."""
        feature_ids = {"pos": [1, 2, 3]}  # No pos_detail_1
        positions = get_disallowed_positions(feature_ids)
    ...
    def test_get_tail_ids_filters_disallowed_positions(self):
        ...
        feature_ids = {
            "pos_detail_1": [10, 20, 30, 40, 50],  # 5 tokens
        }

        # Without filtering
        tail_ids_all = get_tail_ids(feature_ids, "pos_detail_1")
        assert 30 in tail_ids_all

        # With position 2 (value 30) disallowed
        tail_ids_filtered = get_tail_ids(
            feature_ids, "pos_detail_1", disallowed_positions={2}
        )
    ...
    def test_compute_kc_targets_filters_disallowed_from_tail_families(self):
        ...
        feature_ids = {
            "pos_detail_1": [10, 99, 20],  # 99 is disallowed
            "pos": [1, 2, 3],
        }

        targets = compute_kc_targets(feature_ids)

        # TAIL_POS_DETAIL_1 should not contain 99
        tail_pos_detail_1 = targets[KcFamilyId.TAIL_POS_DETAIL_1]
        assert 99 not in tail_pos_detail_1
        # But should contain the other values (10, 20)
        assert 10 in tail_pos_detail_1
        assert 20 in tail_pos_detail_1
    ...
    def test_bag_families_not_affected_by_disallow_list(self):
        ...
        # BAG_POS_DETAIL_1 SHOULD contain 99 (bags are not filtered)
        bag_pos_detail_1 = targets[KcFamilyId.BAG_POS_DETAIL_1]
        assert 99 in bag_pos_detail_1
