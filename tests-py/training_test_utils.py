...
def train_style(test_case, script_path: str, project_root: str, args: str, env_overrides: Optional[Dict[str, str]] = None):
    ...
    test_case.assertEqual(result.returncode, 0, msg=f"Command failed with {result.returncode}.\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}")
    return result
...
def populate_test_data(root_dir: str, project_root: str):
    ...
    for file_path in glob.glob(source_pattern):
        ...
        if not (filename.startswith("jpn_sentences") or filename.startswith("jpn_agrammatic")):
            continue
...
def assert_dir_layout(test_case, root_dir: str, expected_manifest: List[str]):
    ...
    for pattern in resolved_manifest:
        ...
        if not matched:
            test_case.fail(f"Manifest pattern '{pattern}' did not match any files or directories in root.")
    ...
    if remaining_actual:
        ...
        for f in sorted(remaining_actual):
            print(f"  {f}")
        test_case.fail(f"Found {len(remaining_actual)} files/dirs not covered by manifest.")
...
class Bottle:
    ...
    def kotogram_cli(self, *args: str, env_overrides: Optional[Dict[str, str]] = None):
        ...
        self.test_case.assertEqual(result.returncode, 0, msg=f"CLI failed with {result.returncode}.\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}")
        return result
    ...
    def resolve_path(self, path: str) -> str:
        ...
        resolved = path.replace("[.cache]", rel_cache).replace("[data]", rel_data).replace("[models]", rel_models)
    ...
    def assert_dir_diff(self, snap_name: str, expected_diffs: List[str]):
        ...
        for diff in expected_diffs:
            resolved = diff.replace("[.cache]", rel_cache).replace("[data]", rel_data).replace("[models]", rel_models)
        ...
        if unmatched_actual or unmatched_expected:
            ...
            if unmatched_actual:
                msg += "\nUnexpected changes:\n  " + "\n  ".join(sorted(unmatched_actual))
            if unmatched_expected:
                msg += "\nMissing or unmatched expected changes:\n  " + "\n  ".join(sorted(unmatched_expected))
