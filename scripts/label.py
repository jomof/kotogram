...
_CHIVE_VOCAB: frozenset = frozenset()
...
def analyze_batch(
    ...
    surface_to_base_counters: Dict[str, Counter[str]] = collections.defaultdict(Counter)
    ...
    for sentence, gram_label in batch:
        ...
        unknown = [
            vs["surface"]
            for vs, base, lemma in zip(
                token_vocab_strings, token_base_orths, token_lemmas
            )
            if vs["surface"] not in _CHIVE_VOCAB
            and base not in _CHIVE_VOCAB
            and lemma not in _CHIVE_VOCAB
        ]
        if unknown:
            for u in unknown:
                chive_missing_tokens[u] += 1
            chive_skipped.append(sentence)
            continue

        for vs, base in zip(token_vocab_strings, token_base_orths):
            for field in FEATURE_FIELDS:
                vocab_counters[field][vs[field]] += 1
            surface_to_base_counters[vs["surface"]][base] += 1
    ...
    return {
        ...
        "surface_to_base": surface_to_base_counters,
        "stats": label_stats,
...
def worker_p1_wrapper(  # pylint: disable=too-many-positional-arguments
    wid: int,
    chunk: List[Tuple[str, int]],
    s_dir: str,
    overrides: Dict[str, List[Any]],
    result_queue_arg: Any,
    chive_vocab: frozenset,
) -> None:
    global _CHIVE_VOCAB
    if overrides is None:
        overrides = {}
    init_worker(wid, s_dir, overrides, {})
    _CHIVE_VOCAB = chive_vocab
    b_size = 2000
    ...
    buffers: Dict[str, Any] = {
        ...
        "surface_to_base": collections.defaultdict(Counter),
        "stats": {
            "formality": Counter(),
            "gender_prag": Counter(),
            "register": Counter(),
    ...
    for i in range(0, len(chunk), b_size):
        ...
        for k, c in res["surface_to_base"].items():
            buffers["surface_to_base"][k].update(c)
        ...
        cast(List[str], buffers["chive_skipped"]).extend(res["chive_skipped"])
    ...
    result_queue_arg.put(
        (
            buffers["vocab"],
            buffers["surface_to_base"],
            buffers["stats"],
            buffers["reg_samples"],
...
def main() -> None:
    ...
    from train.chive import download_chive as _dl_chive  # pylint: disable=C0415
    from train.chive import load_chive_vocab_set  # pylint: disable=C0415

    console.print("Loading chiVe vocabulary for sentence filtering...")
    _dl_chive()
    chive_vocab = load_chive_vocab_set()
    console.print(f"  chiVe vocabulary: {len(chive_vocab):,} words")
    ...
    merged_surface_to_base: Dict[str, Counter] = collections.defaultdict(Counter)
    ...
    with create_progress(console) as progress:
        ...
        for i, chunk in enumerate(chunks):
            if args.source_db:
                p = ctx.Process(
                    ...
                    args=(i, chunk, shard_dir, result_queue, chive_vocab),
                )
            else:
                p = ctx.Process(
        ...
        while finished_count < len(procs):
            try:
                ...
                for k, counter in s2b.items():
                    merged_surface_to_base[k].update(counter)
    ...
    if all_chive_skipped:
        skip_path = os.path.join(
            train_paths.get_cache_dir(), "chive-unknown-sentences.txt"
        )
        os.makedirs(os.path.dirname(skip_path), exist_ok=True)
        with open(skip_path, "w", encoding="utf-8") as skip_file:
            for s in all_chive_skipped:
                skip_file.write(s + "\n")

        table = Table(
            title="Top 50 missing chiVe tokens (dropped sentences)",
            style="bold red",
        )
        table.add_column("Token", justify="left", style="cyan")
        table.add_column("Drops", justify="right", style="magenta")
        for token, count in merged_chive_missing_tokens.most_common(50):
            table.add_row(token, f"{count:,}")
        console.print(table)

        console.print(
            f"[yellow]Skipped {len(all_chive_skipped):,} sentences with chiVe-unknown tokens[/yellow]"
            f"  → {skip_path}"
        )
    else:
        console.print("[green]All sentences have full chiVe coverage[/green]")
    ...
    final_surface_to_base = {}
    for k, counts in merged_surface_to_base.items():
        if counts:
            final_surface_to_base[k] = counts.most_common(1)[0][0]

    with open(
        os.path.join(dataset_cache_dir, "surface_to_base.json"), "w", encoding="utf-8"
    ) as f_out:
        json.dump(final_surface_to_base, f_out, ensure_ascii=False, indent=2)
    ...
    from train.chive import (
        extract_chive_for_vocab,  # pylint: disable=import-outside-toplevel
    )
    ...
    extract_chive_for_vocab(
        ...
        surface_to_base=final_surface_to_base,
    )
