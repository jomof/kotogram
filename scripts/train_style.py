    ...
    if args.resume and checkpoint is not None:
        ...
        if vocab_grew:
            model.resize_embeddings(
                new_vocab_sizes
            )  # model is already loaded if resume
        ...
        if not args.preprocess_only:
            ...
            MLMTrainer(model, unlabeled_dataset, pretrain_config).train(
                epochs=args.pretrain_epochs, verbose=is_main_process()
            )
        ...
        new_vocab_sizes = tokenizer.get_vocab_sizes()
        if any(new_vocab_sizes[f] > old_vocab_sizes[f] for f in FEATURE_FIELDS):
        ...
        model = (
            StyleClassifierWithMLM(model_config)
            if args.pretrain_kc
            else StyleClassifier(model_config)
        )
