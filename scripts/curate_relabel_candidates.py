...
def _load_sentences_from_db(
    db_path: str,
) -> Tuple[List[str], ValueDict, ValueDict, GPLabelsDict]:
    """Load grammatic sentences from corpus.db."""
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    c.execute(
        "SELECT sentence, formality, gender, grammar, grammar_negative "
    ...
    for sent, f_val, g_val, gp_pos_str, gp_neg_str in rows:
    ...
    return sentences, db_formality, db_gender, db_grammar
...
def _init_inference_cache(cache_path: str) -> None:
    ...
    """)
    # Migrate old table if needed (add gp_probs column)
...
def _load_cached_inference(
...
) -> Tuple[PredictionDict, PredictionDict, GPProbsDict, List[str]]:
    """Load cached inference results. Returns (formality, gender, gp_probs, uncached).
    ...
    # Use fetchmany for better performance
    cols = (
        "sentence, formality_score, formality_pragmatic, gender_score, gender_pragmatic"
    )
    if load_gp_probs:
        cols += ", gp_probs"
    ...
    while True:
        ...
        for row in rows:
            ...
            if load_gp_probs and len(row) > 5 and row[5]:
    ...
    return model_formality, model_gender, model_gp_probs, uncached
...
def _save_inference_cache(
    ...
    for sent in sentences:
        ...
        c.execute(
            ...
            (sent, f_score, int(f_prag), g_score, int(g_prag), gp_probs_json),
        )
...
def _run_inference_with_cache(
    ...
    if uncached:
        ...
        new_formality, new_gender, new_gp_probs = _run_batch_inference(
            uncached, kotograms
        )
        ...
        _save_inference_cache(
            cache_path, uncached, new_formality, new_gender, new_gp_probs
        )
...
def find_relabel_candidates(
    ...
    """
    if families is None:
        families = ["formality", "gender", "grammar_point"]
    ...
    db_data = _load_sentences_from_db(db_path)

    if not db_data[0]:  # Check if sentences list is empty
        console.print("[yellow]No grammatic sentences found in database.[/yellow]")
        return

    console.print(f"Loaded {len(db_data[0]):,} grammatic sentences.")
...
def _validate_relabel_prerequisites(
