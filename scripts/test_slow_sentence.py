...
def test_slow_sentence() -> None:
    ...
    tokens = [Token(extract_token_features(t).surface or t, asdict(extract_token_features(t))) for t in split_kotogram(k)]
    ...
    results = augmenter.filter_grammatical(surfaces, deadline=time.time() + (1.0 - (time.time() - start)))
