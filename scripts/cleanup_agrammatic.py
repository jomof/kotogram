#!/ reentry/env python3
"""
Cleanup script to remove grammatic sentences from agrammatic TSV files.
"""
...
def load_grammatic_sentences(patterns: List[str]) -> Set[str]:
    """Load all unique Japanese sentences from grammatic TSV files."""
    grammatic_sentences = set()
    for pattern in patterns:
        for file_path in glob.glob(pattern):
            print(f"Loading grammatic sentences from {file_path}...")
            with open(file_path, 'r', encoding='utf-8') as f:
                reader = csv.reader(f, delimiter='\t')
                for row in reader:
                    if len(row) >= 3:
                        grammatic_sentences.add(row[2])
    print(f"Total unique grammatic sentences: {len(grammatic_sentences):,}")
    return grammatic_sentences
...
def check_grammaticality_batch(sentences: List[str], parser: SudachiJapaneseParser, model, tokenizer, device: str) -> List[bool]:
    ...
    for s in sentences:
        try:
            ...
            # If parsing fails, we assume it's not grammatic (or at least we can't verify it)
            kotograms.append("")
    ...
    model.eval()
    with torch.no_grad():
        # Predict returns (formality, gender_val, gender_prag, grammaticality, register)
        _, _, _, grammaticality_probs, _ = model.predict(field_inputs, attention_mask)
        # index 1 = grammatic, 0 = agrammatic
        predictions = grammaticality_probs.argmax(dim=-1) == 1
        
    results = predictions.cpu().tolist()
    # Ensure failed ones are marked False
    for i, ids in enumerate(all_feature_ids):
        if ids is None:
            results[i] = False
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Cleanup agrammatic sentences.")
    ...
    # Load grammatic set
    grammatic_set = load_grammatic_sentences([args.grammatic_data])

    # Find agrammatic files
    agrammatic_files = glob.glob(args.agrammatic_data)
    if not agrammatic_files:
        print("No agrammatic files found.")
        return
    ...
    for file_path in agrammatic_files:
        ...
        # Batch analysis
        grammatic_mask = [False] * len(all_rows)
        for i in range(0, len(sentences_to_check), args.batch_size):
            ...
            for j, is_gram in enumerate(batch_results):
                if is_gram:
                    ...
                    grammatic_mask[original_idx] = True
        ...
        removed_count = 0
        for i, row in enumerate(all_rows):
            if i < len(grammatic_mask) and grammatic_mask[i]:
                removed_count += 1
                continue
            # If it was in indices_to_check but model said it's agrammatic, it will be added here
            if i in indices_to_check and not grammatic_mask[i]:
                # We also keep this row
                final_rows.append(row)
            elif i not in indices_to_check:
                # Already in rows_to_keep or just part of standard flow
                final_rows.append(row)
